 ...
 */
package com.yihukurama.app.scanticketdemo.opensource.nohttp.listener;

import android.content.Context;
import android.content.DialogInterface;

import com.iecheck.app.scanticketdemo.R;
import com.yihukurama.app.scanticketdemo.framework.v.widget.Toast;
import com.yihukurama.app.scanticketdemo.framework.v.widget.dialog.WaitDialog;
    ...
    public void onSucceed(int what, Response<T> response) {
        ...
        if (responseCode > 400 && context != null) {
            if (responseCode == 405) {// 405表示服务器不支持这种请求方法，比如GET、POST、TRACE中的TRACE就很少有服务器支持。
                Toast.show(context, R.string.request_method_not_allow);
            } else {// 但是其它400+的响应码服务器一般会有流输出。
                Toast.show(context,response.toString());
            }
        }
        if (callback != null) {
