 ...
 * @author Ryan.Tang
 */
public class YunduanCaptureActivity extends CaptureActivity implements View.OnClickListener {
		...
		requestHandler = new RequestHandler(this){
			...
			protected void yunduanPosAction(YunduanPosBean yunduanPosBean) {
				...
				if(yunduanPosBean.getOutresult().equals("error")){
					Toast.makeText(YunduanCaptureActivity.this, "服务器外层接口错误", Toast.LENGTH_SHORT).show();
					return;

				}

				if(yunduanPosBean.getOutdata().get(0).getResult().equals("ok")){
					Toast.makeText(YunduanCaptureActivity.this, "服务器相应成功，请查看日志；flag是"+yunduanPosBean.getOutdata().get(0).getFlag(), Toast.LENGTH_SHORT).show();
	...
	public void handleDecode(Result result, Bitmap barcode) {
		...
		if (resultString.equals("")) {
			Toast.makeText(YunduanCaptureActivity.this, "Scan failed!", Toast.LENGTH_SHORT).show();
			qrCode = null;
		}else {
